...
review_count = int(args.reviews_count)
...
if review_count:
    data = data[0:review_count]
data['review_clear'] = data['review'].apply(lambda x: ''.join(re.findall('\w+', x)))
...
data['review_indexes'] = data['review_clear'].apply(lambda x: list(map(lambda word: word2index[word], list(x))))
